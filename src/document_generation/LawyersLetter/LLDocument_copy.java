 ...
 */
public class LLDocument_copy extends LLDocument {

	// CONSTRUCTOR
	public LLDocument_copy() {
		super();
	...
	public void writeToDoc(LLSection section) {
		for (LLParagraph p : section.getContents()) {
			...
			for (String s : p.getText().split("%%")) {
				...
				XWPFRun r = ManipDocument.createRun(this.createParagraph());
				// handle italics
				if (s.indexOf('_') >= 0) {
	...
	protected void handleItalics(XWPFParagraph p, String s,
		...
		System.out.println("a --- "+ s.substring(1, underscoreIndex.get(1)));
		...
		System.out.println("b --- "+ s.substring(underscoreIndex.get(i) + 1,
				underscoreIndex.get(i + 1)));
			// repeat
		}
		...
		System.out.println("c --- "+ s.substring(underscoreIndex.get(underscoreIndex.size() - 1) + 1));
		}
	}
