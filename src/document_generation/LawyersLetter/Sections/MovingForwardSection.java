...
public class MovingForwardSection extends LLSection {
    ...
    public MovingForwardSection(LLDocument doc, LLParagraphFactory llpf){
        ...
        //body:
        insertText(llpf,doc,content,ParaCode.REG, "Based on the above, I have advised <client_first_name> that " +
                "<subject_pronoun> has a significant claim for wrongful dismissal damages.  %%" +
                "%%" +
                "Notwithstanding the attached Statement of Claim, and in an effort to resolve the matter in good " +
                "faith, <client_first_name> has advised me that <subject_pronoun> would be willing to settle this matter on the following " +
                "conditions:");

        insertText(llpf,doc,content,ParaCode.LIST, "A lump sum payment of $XX,XXX.XX, representing XX months’ pay in lieu of notice;%%" +
        ...
        insertText(llpf,doc,content,ParaCode.REG, "For a total monetary offer of $XX,XXX.XX at this point in time.%%" +
                "%%" +
